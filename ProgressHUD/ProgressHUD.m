...
{
	[self shared].interaction = YES;
	[[self shared] hudMake:status imgage:image spin:NO hide:YES];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		imgw = image.image.size.width;
		imgh = image.image.size.height;
		
		labelRect.origin.y += imgh;
	...
	CGFloat imagey = (label.text == nil) ? hudHeight/2 : 36+(imgh/2);
